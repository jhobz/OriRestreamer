...
export class StatsComponent implements OnInit {
	...
	ngOnInit() {
		this.socket.on('data', function(data: Information) {
			...
			const runner1 = jQuery.grep(this.players, function(n: any, i) {
				return n.name === data.player1_twitch;
			})[0];

			const runner2 = jQuery.grep(this.players, function(n: any, i) {
				return n.name === data.player2_twitch;
			})[0];

			$.ajax({
				url: 'https://sheets.googleapis.com/v4/spreadsheets/1ZNRh0DrZsY1YMd1EIiEOwmdk-3uGxmTNgX7qamzeozw/values/Stream Stats!'
					+ runner1.startColumn + ':' + runner1.endColumn + '?key=AIzaSyDoT4WSyHDf4a1D0qc6lhdySl92d0tXVG0',
				dataType: 'json',
			...
			$.ajax({
				url: 'https://sheets.googleapis.com/v4/spreadsheets/1ZNRh0DrZsY1YMd1EIiEOwmdk-3uGxmTNgX7qamzeozw/values/Stream Stats!'
					+ runner2.startColumn + ':' + runner2.endColumn + '?key=AIzaSyDoT4WSyHDf4a1D0qc6lhdySl92d0tXVG0',
				dataType: 'json',
				error: function(response) { },
				success: function( response: any ) {
